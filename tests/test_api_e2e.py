...
def model():
    """Model with reasoning prefix (used for both direct and COT modes).

    The reasoning prefix is needed even for direct mode on reasoning models —
    it tells the model to skip thinking and answer immediately.
    """
    prism.set_log_level("info")
    return prism.load_model(MODEL_PATH, backend="mlx", reasoning_prefix=REASONING_PREFIX)
